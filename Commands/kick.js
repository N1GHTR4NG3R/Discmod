...
module.exports = {
	...
	run(bot, message, args) {
        ...
        const kickMsg = embedGen.generatekickEmb(user, message, offender, reason);
        const deniedKick = embedGen.generatedenyEmb(user, message, offender);
        ...
        if(!offender){            
            ...
            if(reason.length === 0){
                message.reply("Please give a reason to kick the user! `/kick (user) (reason)`");
            }else{
