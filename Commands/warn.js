...
module.exports = {
  ...
  run(bot, message, args) {
    ...
    const warnMsg = embedGen.generateUserWarn(
      bot,
      user,
      message,
      offender,
    ...
    if (!offender) {
      ...
      if (reason.length === 0) {
        message.reply(
          `Please give a reason to warn the user! ${
        ...
        message
          .reply("Are you sure you want to warn " + offender.user.username)
          .then((reply) => {
            reply.channel
              ...
              .then((collected) => {
                ...
                let guiMemSQL =
                  "UPDATE GuildMember SET Warnings=Warnings+1 WHERE guild_id = ? AND member_id = ?";
                bot.con.query(guiMemSQL, [updGID, updMem], (err, result) => {
                  if (err) {
                    console.error(
                      err.sqlMessage,
                      "Unable to add warning points to DB! - warn.js line 76"
                    );
                  }
                  return result;
                });
