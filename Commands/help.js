...
module.exports =  {
	...
	run (bot, message, args) {
        ...
        if(args[0] == 'help') return message.channel.send(`Just do /help instead.`); // add code for prefix later!
        ...
        if(args[0]){
            ...
            if(bot.commands.has(command)){
                ...
                const cmdMsg = embedGen.generatecmdEmb(message);
            ...
            fs.readdir(cmds, (err, files) => {
                ...
                // Create embed
                const embedGen = new embGen();
                const helpMsg = embedGen.generatehelpEmb(message);
                // Add to embed
                helpMsg.addField('**__Info:__**', `To get a more detailed explanation of each command please use /help <command>.`), // add code for prefix later!
                helpMsg.addField('**__Commands:__**', `${command}`, true),
                helpMsg.addField('**__Logs:__**', `**Voice**: ${logs.Voice}\n **Messages**: ${logs.Messages}\n **Members**: ${logs.Members}`, true)
                // Send Embed
