...
module.exports = async (bot, guild) => {
  // Check Permissions
  if (guild.me.hasPermission(["MANAGE_CHANNELS", "MANAGE_ROLES", "KICK_MEMBERS", "BAN_MEMBERS", "CREATE_INSTANT_INVITE",
      "MANAGE_CHANNELS", "MANAGE_GUILD", "ADD_REACTIONS", "VIEW_AUDIT_LOG", "VIEW_CHANNEL", "SEND_MESSAGES", "MANAGE_MESSAGES",
      "EMBED_LINKS", "ATTACH_FILES", "READ_MESSAGE_HISTORY", "MENTION_EVERYONE", "VIEW_GUILD_INSIGHTS", "MUTE_MEMBERS", "MOVE_MEMBERS",
  ...
  const validPerms = validChannels.filter(ch => ch.permissionsFor(bot.user).has('SEND_MESSAGES'));
  const defaultChannel = validPerms.reduce((last, ch) => {
    if (last == null) return ch;
    return ch.createdAt > last.createdAt
      ? ch
      : last;
  }, null);

  // Output a Message to direct a user to a website, for configuration.
	const embedGen = new embGen();
  const welcomeMsg = embedGen.generatewelMsg(bot);
  ...
  bot.con.query(search, checkID, (err, res) => {
    if (err) {
      console.error(err,"There was an error Insering into the guild to the DB!");
    }
    if (res.length === 0) {
  ...
  await bot.con.query("SELECT G.guild_id, G.guild_name, G.guild_owner, G.guild_owner_id, G.guild_member_count, G.guild_prefix FROM Guilds G", (err, result) => {
    if (err) {console.error('Unable to get Guild data Table: Index.js - Line 56')}
    console.table(result);
    console.log(cc.green(`${GuildInfo.guildName} Just added Contender!`))
  })
  ...
  await bot.con.query(sql, GuildInfo.guildID, (err, result) => {
    if (err){console.error('Unable to get Default Channel! - guildCreate.js - Line 80')}
    return result;
  })
}