...
module.exports = async (bot, member) => {
  ...
  bot.con.query(sql, updGuildMem, function (err, result) {
    if (err) {
      console.error(
        err,
        "There was an error Updating the guild in the DB! - guildMemberRemove.js line 17"
      );
    }
    console.log(
      `Removed ${MemberInfo.userName} from ${MemberInfo.userGuildName}`
    );
    return result;
  });
  ...
  // Output the log
  if (member.user.bot) {
    console.log(`${member.user.tag} just left ${MemberInfo.userGuildName}`);
  } else {
