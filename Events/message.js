...
module.exports = (bot, message, guild) => {
    ...
    getPrefix((err, result) => {
        ...
        result.forEach((item, index) => {
            // Match guild ID with database
            if(message.guild.id === item.ID){
                // Check the Prefix, Ignore if User is a bot!
                if(!message.content.startsWith(item.Prefix) || message.author.bot) return;
                
                // Remove prefix and put commands into lowercase for ease
                const args = message.content.slice(item.Prefix.length).split(/ +/);
